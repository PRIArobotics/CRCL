...
async function runAll(){
    ...
    addToQueue(festo, new CRCLCommand("SetEndEffectorParameters","Using VacuumGripper2mm", {"ToolID": 2}))
    ...
    for (let i of [3, 4, 5,   7, 8]){
        ...
        addToQueue(festo, new CRCLCommand('SetEndEffector',"Picking Target"+i,{"Setting": 0.0}));
        addToQueue(festo, new CRCLCommand('MoveTo', 'Move high above Target'+i, {"Straight":false,"Pose":setHeight(target, festoSafetyHeight)}))
    ...
    const groups = groupByRobot(queue)
    ...
    for (const g of groups){
        ...
        //await this.robots[currentRobot].schedule(cmdBuffer)
    }
}
